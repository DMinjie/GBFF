...
print("The training method is coming soon!")
train_dataset = Dataset.FusionDataset("Trainset/MEF/Source_A", "Trainset/MEF/Source_B", patch_size=PATCH_SIZE, stride=STRIDE)
train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True)

# === Model ===
model = GNet.GNet().to(DEVICE)
...
for epoch in range(EPOCHS):
    ...
    for x, y in loop:
        ...
        for i in range(x.size(0)):
            ...
            # == Granual ball  ==
            fused_pil, q_ratio, w_ratio =GBPC.GBPC(pil_x, pil_y)
        ...
        POS_Ratio = torch.tensor(W_list, device=DEVICE).unsqueeze(1).unsqueeze(2).unsqueeze(3)
        ...
        # == POS and BND guide==
        sobel_all = POS_Ratio* sobel_target +BND_Ratio* (sobel_x + sobel_y)
        ...
        ssim_value_2= ssim_metric(output, x).to(DEVICE)
        ssim_value_3= ssim_metric(output, y).to(DEVICE)

        loss=(1-ssim_value_1)+loss_y_pout+loss_x_pout+sobel_loss
